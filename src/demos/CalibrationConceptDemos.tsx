...
export function KalibreringskurveDemo() {
  return (
    ...
    >
      <svg viewBox="0 0 520 250" className={shared.svg} role="img" aria-label="Fem kalibreringspunkter langs en stigende kalibreringslinje">
        ...
        {[[90,183],[170,156],[250,128],[330,100],[430,66]].map(([x,y]) => <circle key={`${x}-${y}`} cx={x} cy={y} r="7" className={styles.point} />)}
  ...
  return (
    ...
    >
      <ValueRows rows={[
        ...
        { label: "Mellom LOD og LOQ", values: "tilstedeværelse kan være påvisbar, men tallverdien er ikke god nok" },
  ...
  return (
    ...
    >
      <div className={styles.formulaLine}>
        <span>A: 500 / 10 = 50</span><span>B: 300 / 10 = 30</span><strong>A responderer 1,67× sterkere</strong>
      </div>
    </DemonstrationFrame>
  );
}
  ...
  return (
    ...
    >
      <div className={styles.rangeWrap}>
        <div className={styles.rangeLabels}><span>LLOQ</span><span>arbeidsområde</span><span>ULOQ</span></div>
        <div className={styles.rangeBar}><span className={styles.rangeActive} /></div>
      </div>
    </DemonstrationFrame>
  );
}
...
export function NullpunktDemo() {
  return (
    ...
    >
      <div className={styles.formulaLine}><strong>y = a + bx</strong><span>x = 0 → y = a</span><span>a = konstantledd / skjæring</span></div>
    </DemonstrationFrame>
  );
}
...
export function DriftDemo() {
  ...
  return (
    ...
    >
      <svg viewBox="0 0 520 220" className={shared.svg} role="img" aria-label="Punkter som gradvis stiger gjennom analyseserien">
        <line x1="45" y1="180" x2="490" y2="180" className={styles.axis} />
        <line x1="45" y1="180" x2="45" y2="25" className={styles.axis} />
        {points.map((value, index) => <circle key={`${value}-${index}`} cx={70 + index * 55} cy={165 - (value - 100) * 12} r="6" className={styles.point} />)}
...
export function KontrollproveDemo() {
  return (
    ...
    >
      <Flow items={["Kalibrer serien", "Analyser kontrollprøven", "Sammenlign med kontrollkrav", "Godta, undersøk eller stopp serien"]} />
    </DemonstrationFrame>
  );
}
...
export function EksternKalibreringDemo() {
  return (
    ...
    >
      <Flow items={["Lag separate kalibratorer", "Mål kalibratorserien", "Etabler kalibreringsrelasjonen", "Bruk relasjonen på prøve responsen"]} />
    </DemonstrationFrame>
  );
}
  ...
  return (
    ...
    >
      <div className={styles.columns}>
        <div><span className={styles.cardTitle}>Løsemiddel</span><strong className={styles.signal}>100</strong><p className={shared.note}>respons ved kjent nivå</p></div>
        <div><span className={styles.cardTitle}>Prøvematriks</span><strong className={styles.signal}>72</strong><p className={shared.note}>samme nivå, men matrise undertrykker responsen</p></div>
        <div><span className={styles.cardTitle}>Matrikstilpasset standard</span><strong className={styles.signal}>73</strong><p className={shared.note}>kalibratoren etterligner prøveeffekten</p></div>
      </div>
    </DemonstrationFrame>
  );
}
