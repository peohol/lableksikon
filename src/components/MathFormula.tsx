...
export function MathFormula({
  ...
  const queueLatestRender = useCallback(() => {
    ...
    const queued = enqueueMathJax(async (mathJax) => {
      try {
        ...
        const target = targetIndex === 0 ? firstRef.current : secondRef.current;
        if (!target) return;
        ...
        if (mountedRef.current && requestedTexRef.current !== sourceTex) {
          queueMicrotask(queueLatestRender);
        }
      }
    });
  ...
  useEffect(() => {
    ...
    if (queueLatestRender()) return;

    const renderWhenReady = () => queueLatestRender();
    window.addEventListener(MATHJAX_READY_EVENT, renderWhenReady, { once: true });
    return () => window.removeEventListener(MATHJAX_READY_EVENT, renderWhenReady);
  }, [queueLatestRender, tex]);
